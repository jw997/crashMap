...
//var data = await readCsv(fs);
/*
async function readCsvs(fileName) {
	const papaPromise = (st) => new Promise((resolve, reject) => {

		Papa.parse(st, {
			header: true,
			complete: function (results) {
				console.log('ciomplete')
				resolve(results);
			},
			error: function (error) {
				console.log('error')
	...
	const results = await papaPromise(readStream);
}

const st = fs.createReadStream("./ccrs2025crashes.csv");
...
Papa.parse(st, config);


async function readCsv(importFile) {
	const papaPromise = (importFile) => new Promise((resolve, reject) => {
		const file = fs.createReadStream(importFile);
		Papa.parse(file, {
			header: true,
